...
def reflect_list(wall_1side_list, inout_strands, inout_strands_index, p):
    ...
    if len(wall_1side_list) == ((p-1)/2)+1:  # base casep
        ...
        if inout_strands_index == 0:
            ...
            # ,inout_strands, 1, p)
            wall_1side_list.append(reflect(wall_color, inout_strands[inout_strands_index], p))
...
def wall_colors(overstrand_list, color_list, p):
    ...
    # if the color of the overstrand does not match the color of the incoming understrand
    for crossing in range(len(overstrand_list)):
        if color_list[overstrand_list[crossing]] != color_list[crossing]:  # heterogenous
            ...
            heterogenous_wall_list.append(reflect_list(
                [color_list[overstrand_list[crossing]]], incomingoutgoing_list[crossing], 0, p))
            heterogenous_wall_list.append(reflect_list(
                [color_list[overstrand_list[crossing]]], incomingoutgoing_list[crossing], 1, p))
            ...
            homogeneous_wall_list.append([reflect(color, homogeneous_color, p)
                                          for color in homog_index_list])
